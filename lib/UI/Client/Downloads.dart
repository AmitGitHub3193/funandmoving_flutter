...
class Downloads extends StatefulWidget with WidgetsBindingObserver {
  @override
...
class _DownloadsState extends State<Downloads> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        leading: Container(),
        centerTitle: true,
        title: MyText(title:'Favorites',clr: Colors.white,),
        actions: [
          if(downloadsListMaps!=null && downloadsListMaps.isNotEmpty)
          GestureDetector(onTap: (){
            if(downloadsListMaps.isNotEmpty) {
              downloadsListMaps.forEach((element) {
                FlutterDownloader.remove(
                    taskId: element["id"], shouldDeleteContent: true);
              });
              services.removeAllVideos();
              setState(() {
      ...
      body: downloadsListMaps.length == 0
        ...
        child: ListView.builder(
          ...
          itemBuilder: (BuildContext context, int i) {
            ...
            File? _file =
            _directories.isNotEmpty ? _directories.first as File? : null;
  ...
  Widget buttons(DownloadTaskStatus? _status, String? taskid, int index) {
    void changeTaskID(String? taskid, String? newTaskID) {
      Map task = downloadsListMaps.firstWhereOrNull(
